...
The entire user interface was redesigned with a glassmorphic aesthetic. All panels, sidebars, toolbars, buttons, and tooltips now use semi-transparent backgrounds with \texttt{backdrop-filter: blur()} and subtle border highlights (\texttt{rgba(255,255,255,0.3)}), creating a frosted-glass effect. The colour scheme is exclusively light-themed: dark theme was removed entirely (\texttt{theme-toggle.js} and \texttt{theme-toggle.css} deleted; \texttt{theme-config.js} defaults to \texttt{"light"} and its toggle function is a no-op). All graph strokes --- KG edges, network links, arrow markers, node outlines --- were changed from white-based RGBA to black-based RGBA (\texttt{rgba(0,0,0,0.15)} to \texttt{rgba(0,0,0,0.25)}) to remain visible on the light background. Sidebar section fonts, padding, and dot sizes were increased approximately 40\% for improved readability. The redesign is fully contained in \texttt{css/style.css} with no external dependencies.

...
\begin{abstract}
This report documents the architecture, data model, implementation, and recent improvements of the \emph{Dark Series Knowledge Graph Visualization} --- an interactive web application for exploring the temporal, causal, and character-based relationships of the television series \emph{Dark} (Netflix, 2017--2020). The application transforms flat CSV data into a typed Knowledge Graph and provides seven coordinated visualization views, each projecting the same underlying graph from a different perspective. This report details the data model, the D3.js-based rendering pipeline, the bug fixes and architectural improvements applied, and guidelines for extending the system.
\end{abstract}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Chord Diagram} --- a circular co-occurrence diagram showing character pair relationships across events.
...
\end{enumerate}
This correctly links all Dark identity pairs: Jonas/Adam, Mikkel/Michael, Martha/Eve,\linebreak Noah/Hanno Tauber, and cross-world duplicates (e.g., Charlotte Doppler in both Jonas and Martha worlds).
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshot/01-knowledge-graph.png}
\caption{Knowledge Graph view showing typed nodes and causal edges}
\label{fig:kg}
\end{figure}
...
\textbf{Edges:} Each relation type has a distinct color, dash pattern, and marker (colors use black-based RGBA for visibility on the light theme background):
\begin{itemize}[nosep]
    \item \texttt{causes}: solid dark arrow, width 1.5
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshot/02-temporal.png}
\caption{Temporal Graph with swimlanes and causal Bezier curves}
\label{fig:temporal}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshot/03-network.png}
\caption{Force Network showing event interconnectedness via shared characters}
\label{fig:network}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshot/04-barchart.png}
\caption{Bar Chart with configurable grouping and metrics}
\label{fig:barchart}
\end{figure}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Edge Type Distribution} --- a donut chart of causal edge types (Normal, World Swap, Successful Time Travel, Failed Time Travel, Quantum Entanglement, Same Event, Inconclusive) using D3's \texttt{d3.pie} layout.
...
\subsection{Timeline (\texttt{timeline.js})}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshot/07-timeline.png}
\caption{Timeline with beeswarm layout across World bands}
\label{fig:timeline}
\end{figure}
...
The BFS shortest-path algorithm was extracted into a dedicated \texttt{kg\_pathfinder.js} module (17 lines). This improves separation of concerns and makes pathfinding independently testable. \texttt{kg\_view.js} now calls \texttt{KGPathfinder.shortestPath()} instead of \texttt{KGBuilder.shortestPath()}.

\subsection{UI Redesign: Glassmorphic Theme}
The entire user interface was redesigned with a glassmorphic aesthetic. All panels, sidebars, toolbars, buttons, and tooltips now use semi-transparent backgrounds with \texttt{backdrop-filter: blur()} and subtle border highlights, creating a frosted-glass effect. The colour scheme is exclusively light-themed: dark theme was removed entirely (\texttt{theme-toggle.js} and \texttt{theme-toggle.css} deleted). All graph strokes --- KG edges, network links, arrow markers, node outlines --- were changed from white-based RGBA to black-based RGBA (opacity 0.15--0.25) to remain visible on the light background. Sidebar fonts, padding, and dot sizes were increased by roughly 40\% for improved readability.

\subsection{Bug Fix: Edge Stroke Visibility on Light Background}
\label{sec:lightfix}
\textbf{Files:} \texttt{kg\_view.js}, \texttt{network.js}, \texttt{chord.js}, \texttt{css/style.css}

\textbf{Problem:} After switching to the light theme, all graph edges, arrow markers, and node strokes inherited the old dark-theme colour \texttt{rgba(255,255,255,...)}, making them invisible against the white background.

\textbf{Fix:} Every rendered stroke was systematically updated:
\begin{itemize}[nosep]
    \item KG edge fallback stroke: \texttt{rgba(255,255,255,0.20)} $\rightarrow$ \texttt{rgba(0,0,0,0.20)}
    \item Event node stroke: \texttt{rgba(255,255,255,0.15)} $\rightarrow$ \texttt{rgba(0,0,0,0.15)}
    \item Network arrow marker fill: \texttt{rgba(255,255,255,0.25)} $\rightarrow$ \texttt{rgba(0,0,0,0.25)}
    \item Network node stroke: \texttt{rgba(255,255,255,0.15)} $\rightarrow$ \texttt{rgba(0,0,0,0.15)}
    \item Chord ribbon and arc strokes: changed to use CSS custom properties (\texttt{--text-primary}) for automatic theme adaptation.
\end{itemize}
...
The KG was built as the central data abstraction layer. The \texttt{KGBuilder} module converts the flat event-and-edge CSV data into a typed graph with explicit node and edge objects. The identity detection algorithm (described in Section~\ref{sec:datamodel}) matches \texttt{same\_person\_as} pairs by stripping world-market tags and checking for word-level name overlap. The D3 mutation protection fix (detailed in Section~\ref{sec:mutationfix}) stores persistent \texttt{sourceId} and \texttt{targetId} strings to avoid reliance on D3-mutated link properties.
...
\begin{itemize}[nosep]
    ...
    \item The \textbf{Chord Diagram} reveals character co-occurrence patterns through a circular layout, highlighting which character pairs appear together most frequently across events.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Performance with 800+ nodes and 2000+ edges}: The KG force simulation uses type-specific charge strengths (World nodes repel strongly, Characters moderately, Events\-/Time\-Periods weakly) and clustering forces to keep the graph readable. Filter re-renders are full rebuilds rather than incremental updates, which is acceptable for the current dataset size.
...
\bibitem{d3}
Mike Bostock,
\textit{D3.js -- Data-Driven Documents},
\url{https://d3js.org/}

\bibitem{dark}
Baran bo Odar \& Jantje Friese,
\textit{Dark} (TV series, 2017--2020),
Netflix.

\bibitem{chord}
M. Bostock,
\textit{D3 Chord Diagram},
Observable. \url{https://observablehq.com/@d3/chord-diagram}

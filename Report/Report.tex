...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshot/07-chord.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Screenshot/06-timeline.png}
...
\subsection{Feature Addition: Chord Dependency Diagram}
A seventh visualization view was added --- a circular chord diagram (\texttt{chord.js}) showing character co-occurrence across events. The module builds a symmetric co-occurrence matrix from the event--character adjacency data, retains the top 14 most frequent characters, and renders the result using D3's \texttt{d3.chord()} layout. Directed ribbons distinguish source and target arcs, and a hover interaction provides detailed co-occurrence counts. The diagram uses CSS custom properties for colour theming and applies \texttt{blendMode="multiply"} for correct compositing on the light background. This view fills a relational gap not addressed by the existing event- and timeline-oriented views.
